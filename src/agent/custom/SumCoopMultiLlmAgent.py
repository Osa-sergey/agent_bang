...
class SumCoopMultiLlmAgent(Agent):
...
Your co**: Players can exchange messages to coordinate or mislead.
mmunication should be strategic, multi-layered and take into account the psychology of the players.
...
Summarize the information received and answer the question as correctly as possible. Use your previous thoughts for this purpose.
    ...
    def ask_llm(self, prompt: str, agents: list, state: Union[str, None] = None) -> Union[Dict[str, Any], str]:
        ...
        if state:
            ...
            task_context.append({"role": "user", "content": state})
        else:
        ...
        for i, agent in enumerate(agents):
            ...
            response = self.client.chat.completions.create(
                ...
                messages=system_prompt + self.chat_context + task_context,
                temperature=0.7,
                max_tokens=700,
                stream=False
            )
        ...
        response = self.client.chat.completions.create(
            model="deepseek-chat",  # deepseek-reasoner или deepseek-chat
            messages=self.summary_system_prompt + self.chat_context + task_context,
            temperature=0.7,
            max_tokens=700,
            stream=False
        )
        ...
        print(f"Summary")
        print("summary")
        self.chat_context.append({"role": "assistant", "content": summary})
