...
class BaseLlmAgent(Agent):
    def __init__(self, agent_name: str,
        ...
        self.__console = Console(force_terminal=True)
        self.__client = self.__init_agent()
        self.__prompts = BaseLlmAgentPrompts()
        self.system_prompt = self.__prompts.system_prompt
        self.chat_context = [
            {"role": "system", "content": self.system_prompt}
        ]
        self._errors = 0
        super().__init__(agent_name, config, player, game, shared_memory)
        self.CONTEXT_LEN = self.agent_config["context_len"]

    @staticmethod
    def __init_agent(agent_type: AgentType = AgentType.DEEPSEEK):
        client = None
        match agent_type:
    ...
    def ask_llm(self, prompt: dict[str, str], so_answer_field_name: Union[str, None] = "result") -> str:
        ...
        while (errors
            ...
            prompt = {"prompt": self.__prompts.get_regenerate_prompt(so_answer_field_name, errors)}
            answer, json_objects, errors = self.generate_answer(prompt, regenerate=True)

        result = json_objects[0].get(so_answer_field_name) if so_answer_field_name else answer
        self.__console.print(f"[green]LLM answer:[/green] \n{result}", style="bold")
        return result

    def generate_answer(self, prompt: dict[str, str], regenerate: bool = False):
        prompt = prompt['prompt']
        self.__console.print(f"[blue] Prompt: \n{prompt} [/blue]", style="bold")
    ...
    def trim_chat_context(self):
        if len(self.chat_context) > self.CONTEXT_LEN:
            print("===" * 30)
            print("Trim chat context")
            print("===" * 30)
            old_context = self.chat_context[-self.CONTEXT_LEN:]
            self.chat_context = [
                {"role": "system", "content": self.system_prompt}
            ]
    ...
    def extract_json_objects(self, row_text: str):
        ...
        for match in matches:
            ...
            try:
                ...
                self.__console.print(f"[green]Parsed JSON:[/green]", style="bold")
                pprint.pprint(json_object)
                print("===" * 30)
            except json.JSONDecodeError as e:
                self.local_log.append({"content": row_text, "error": e})
                self.__console.print(f"[red]ERROR:[/red] JSON parsing error in block: {e}", style="bold")
                errors.append(str(e))
    ...
    def choice_card_for_play(self) -> str:
        ...
        prompt = self.__prompts.choice_card_for_play_prompt(game_state=game_state)
    ...
    def get_opponent(self, card: Card) -> str:
        ...
        prompt = self.__prompts.get_opponent_prompt(game_state=game_state)
        return self.ask_llm(prompt)

    def get_action_type(self, card: Card, options: dict) -> str:
        opponent = options["opponent"]
        game_state = {"card": card, "opponent": opponent}
        prompt = self.__prompts.get_action_type_prompt(game_state=game_state)
        return self.ask_llm(prompt)
    ...
    def get_card_for_steal(self, card: Card, options: dict) -> str:
        ...
        prompt = self.__prompts.get_card_for_steal_prompt(game_state=game_state)
        return self.ask_llm(prompt)

    def get_indians_response(self) -> str:
        game_state = self.get_game_state()
        prompt = self.__prompts.get_indians_response_prompt(game_state=game_state)
        return self.ask_llm(prompt)

    def get_bang_response(self) -> str:
        game_state = self.get_game_state()
        prompt = self.__prompts.get_bang_response_prompt(game_state=game_state)
        return self.ask_llm(prompt)

    def get_gatling_response(self) -> str:
        game_state = self.get_game_state()
        prompt = self.__prompts.get_gatling_response_prompt(game_state=game_state)
        return self.ask_llm(prompt)

    def get_card_for_discard(self, num_cards: int) -> str:
        if self._errors < 3:
            game_state = {"num_cards": num_cards, "cur_state": self.__get_player_current_state()}
            prompt = self.__prompts.get_card_for_discard_prompt(game_state=game_state)
            return self.ask_llm(prompt)
        else:
    ...
    def react_to_discard_error(self, errors: str):
        ...
        prompt = self.__prompts.react_to_discard_error_prompt(game_state=game_state)
        self.ask_llm(prompt, so_answer_field_name=None)
        self.__console.print(f"[red]ERROR:[/red] ERROR ON DISCARD", style="bold")
        self._errors += 1
